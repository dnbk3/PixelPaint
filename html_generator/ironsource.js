    ...
    window.onload = function(){    
        ...
        function adVisibleCallback(event){
          ...
          if (event.isViewable){
            ...
            const audioEvent = new CustomEvent("audioChanged", { detail: { mute: true } });
            audioEvent.detail.mute = true;
        ...
        function audioVolumeChangeCallback(volume){
          ...
          if (isAudioEnabled){
            //START or turn on the sound function boot ()
            setTimeout(() => {
              const audioEvent = new CustomEvent("audioChanged", { detail: { mute: true } });
            audioEvent.detail.mute = false;
            window.dispatchEvent(audioEvent);
            }, 500);
          } else {
