...
def writeNetHtml(js, html,name="index.html"):
	...
	if not os.path.exists(f"./GA_V1/{net}"):
		os.makedirs(f"./GA_V1/{net}")
	print(f"create html for: {net}")
	newpath = f"./GA_V1/{net}/{name}"
...
def integrate(projectRootPath):
    ...
    for js in additonjs:
        if js.lower()=="pangle.js":
          writeNetHtml(js, htmlStr, "index.html")
        elif "vungle.js" in js.lower():
          writeNetHtml(js, htmlStr, "ad.html")
        else:
          writeNetHtml(js, htmlStr, "PixelPaint_Ply_V10_250324"+js.split(".js")[0].capitalize()+".html")
